...
int calculateSurgeAndYaw() {
    ...
    if (distance < MINTH) {
        ...
        surge = (distance - MINTH) * MAX_PWM / (MAXTH - MINTH); // Proportional law
        yaw = MAX_PWM - surge;
        move_state = Move_Forward;
    }
    return move_state;
...
void controlMotors() {
    ...
    // Set PWM values based on move state
    switch (move_state) {
        case Move_Forward:
            setPWM( motorLB, 0);
...
int next_value(const char* msg, int i) {
    while (msg[i] != ',' && msg[i] != '\0') { i++; } // Wait for the comma
        if (msg[i] == ',')
            i++;
    return i;
}
...
void pcth(const char* msg){
    ...
    if (newMINTH < newMAXTH){ // check the validity of the new values
        MINTH = newMINTH;
        MAXTH = newMAXTH;
    }
}
...
void reciver(){
    if(CirBufRx.maxlen > 0){
            ...
            if(ret == NEW_MESSAGE){
                if(strcmp(pstate.msg_type, "PCTH") ==0){ // check the type of the message
...
void scheduler() {
    int i;
    for (i = 0; i < MAX_TASKS ; i++) { // Loop through all tasks
        schedInfo[i].n++;
        if (schedInfo[i].n >= schedInfo[i].N) { // If it's time to execute the task
            switch(i) { // Execute each task based on the frequency of the task
                case 0: // 1 KHz
                    disCalc() ;
                    if(stateFlag){
                        controlMotors();
                    ...
                    if(CirBufTx.maxlen != 0){UARTTX(&CirBufTx);} 
                    pbHandller();
                    ledHandler();
                    reciver();
                    break;   
